...
logging.basicConfig(filename='ErrorLogs.log', level=logging.ERROR, format='%(asctime)s %(levelname)s %(message)s')
...
def get_args(api_account_id, access_token, base_path, email, name):
    signer_email = email
    signer_name = name
    cc_email = DS_JWT["cc_email"]
    cc_name = DS_JWT["cc_name"]
...
def run_example(private_key, api_client, email, name):
    jwt_values = get_token(private_key, api_client)
    args = get_args(jwt_values["api_account_id"], jwt_values["access_token"], jwt_values["base_path"], email, name)
...
def main():
    ...
    try:
        run_example(private_key, api_client, sys.argv[1], sys.argv[2])
